...
fn test_mcap_extreme_compression_ratio() {
    ...
    assert!(compressed.len() < 1000, "Highly compressible data should compress well");
}
...
fn test_compression_level_zstd_mappings() {
    ...
    for (level, expected) in levels {
        assert_eq!(level.to_zstd_level(), expected, "Zstd level mismatch for {:?}", level);
    }
}
...
fn test_compression_level_lz4_mappings() {
    ...
    for (level, expected) in levels {
        assert_eq!(level.to_lz4_level(), expected, "LZ4 level mismatch for {:?}", level);
    }
}
...
fn test_all_compression_type_combinations() {
    ...
    for compression_type in &types {
        for compression_level in &levels {
            ...
            let result = serializer.serialize_batch("test_topic", samples, "test_recording");
            assert!(result.is_ok(), "Failed with {:?}/{:?}", compression_type, compression_level);
        }
    }
}
...
fn test_mcap_with_special_characters_in_topic() {
    ...
    let samples = vec![create_sample(vec![1, 2, 3])];
    let result = serializer.serialize_batch("topic/with/special_chars-123", samples, "test-recording-001");
...
fn test_mcap_no_compression_size() {
    ...
    assert!(output.len() > 10000, "No compression should preserve most size");
}
...
fn test_mcap_compression_type_none_vs_lz4() {
    ...
    let result1 = no_compression.serialize_batch("test", samples1, "rec").unwrap();
    let result2 = lz4_compression.serialize_batch("test", samples2, "rec").unwrap();

    assert!(result2.len() < result1.len(), "LZ4 should compress better than none");
}
...
fn test_mcap_compression_type_lz4_vs_zstd() {
    ...
    assert!(result2.len() < result1.len(), "Zstd should compress better than LZ4 for repetitive data");
}
