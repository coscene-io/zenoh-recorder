...
async fn test_storage_ensure_bucket() {
    if !is_reductstore_available().await {
        eprintln!(
            "Skipping test: ReductStore not available at {}",
            get_reductstore_url()
        );
        return;
    }
...
async fn test_storage_write_with_special_characters_in_labels() {
    ...
    assert!(
        result.is_ok(),
        "Failed to write with special label names: {:?}",
        result.err()
    );
}
...
async fn test_storage_write_with_various_timestamps() {
    ...
    let test_cases = vec![
        (
            test_entry_name("timestamp_old"),
            current_timestamp_us() - 1_000_000_000,
        ), // 1000 seconds ago
        (test_entry_name("timestamp_now"), current_timestamp_us()),
        (
            test_entry_name("timestamp_future"),
    ...
    for (i, (entry_name, timestamp_us)) in test_cases.into_iter().enumerate() {
        ...
        assert!(
            result.is_ok(),
            "Failed to write with timestamp {}: {:?}",
            i,
            result.err()
        );
    }
...
async fn test_storage_write_with_long_entry_name() {
    ...
    assert!(
        result.is_ok(),
        "Failed to write with long entry name: {:?}",
        result.err()
    );
}
